...
public:
	...
	bool operator== (ArraySequence<T> y);
	bool operator!= (ArraySequence<T> y);
...
template <typename T>
bool ArraySequence<T>::operator== (ArraySequence<T> y) {
...
template <typename T>
void siftdown(ArraySequence<T>* seq, int n, int i, int (*cmp)(T, T))
{
	int largest = i;
	int l = 2 * i + 1;
	int r = 2 * i + 2;
	...
	if (largest != i)
	{
		...
		(*seq)[i] = (*seq)[largest];
		(*seq)[largest] = tmp;
		siftdown(seq, n, largest, cmp);
	}
}

template <typename T>
void HeapSort(ArraySequence<T>* seq, int (*cmp)(T, T))
{
	int n = seq->GetSize();
	for (int i = n / 2 - 1; i >= 0; i--)
		siftdown(seq, n, i, cmp);
